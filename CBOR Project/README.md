...
```
Container Data → Remove Keys → Delimited String → CBOR Compress → HTTP POST
                                                                        ↓
Reconstruct Keys ← Split Delimiter ← CBOR Decompress ← Queue Processing ← Receive
```
...
```bash
# Python dependencies
pip install locust cbor2 requests
