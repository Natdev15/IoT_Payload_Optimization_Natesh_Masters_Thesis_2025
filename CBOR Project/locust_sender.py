...
class ContainerDataSender(HttpUser):
    ...
    def initialize_data_pool(cls):
        ...
        logger.info(f"   [{worker_label}] Compression method: Pure CBOR")
        ...
        logger.info(f"   CBOR size - Avg: {avg_size:.1f}B, Min: {min_size}B, Max: {max_size}B")
...
def on_test_start(environment, **kwargs):
    ...
    logger.info(f"Compression method: Pure CBOR")
...
def test_compression():
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write("- Use 'cbor' module for decompression\n")
        f.write("- Direct CBOR decode to JSON\n")
