...
// Pure MessagePack compression: direct JSON to MessagePack encoding
...
// Test Pure MessagePack compression and decompression
function testCompression() {
    console.log('Testing Pure MessagePack Compression...');
    ...
    console.log('Testing Pure MessagePack compression...');
    const compressedData = msgpackCompress(originalData);
    console.log(`Compressed data size: ${compressedData.length} bytes`);
    console.log(`Compressed data (hex): ${compressedData.toString('hex')}`);
    console.log();
    
    // Test Pure MessagePack decompression
    console.log('Testing Pure MessagePack decompression...');
    try {
        const decompressedData = msgpackDecompress(compressedData);
    ...
    console.log(`MessagePack: ${compressedData.length} bytes`);
...
async function runAllTests() {
    console.log('Pure MessagePack Compression Test Suite');
    console.log('='.repeat(50));
    ...
    const filename = `pure_msgpack_compression_test_nodejs_${timestamp}.txt`;
    
    let output = '';
    output += 'PURE MESSAGEPACK COMPRESSION TEST RESULTS (Node.js)\n';
    output += '='.repeat(50) + '\n';
    output += `Test Date: ${new Date().toISOString()}\n`;
    output += `Compression Method: Pure MessagePack\n`;
    ...
    output += `Pure MessagePack: ${testResults.compressedData.length} bytes\n`;
    ...
    output += 'MESSAGEPACK COMPRESSED DATA:\n';
    output += '-'.repeat(30) + '\n';
    ...
    output += '-'.repeat(30) + '\n';
    try {
        const decompressed = decode(testResults.compressedData);
